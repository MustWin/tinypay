...
module.exports = {
	...
	getStatusPage: function(response, status_code, file_extension, header, request_options) {
		...
		self.cacheStore.get(status_code_path, file_extension, ObjectUtils.cacheObj(null, header), request_options, function(err, cache_obj){
			if(!err){
				...
				Renderer.render(status_code_path, file_extension, null, request_options, function(rendered_obj) {
					...
					if (body_length) {
						self.cacheStore.update(status_code_path, file_extension, ObjectUtils.cacheObj(rendered_obj.body, header), request_options, function(err){
							if (err) {
								// we do nothing here.
	...
	prepareRenderedResponse: function(response, request_basename, file_extension, rendered_obj, request_options) {
		...
		if (status_code === 200 && rendered_obj.body.length) {
			self.cacheStore.update(request_basename, file_extension, ObjectUtils.cacheObj(rendered_obj.body, header), request_options, function(err, cache_obj) {
				if (err) {
	...
	prepareCachedResponse: function(response, request_basename, file_extension, rendered_obj, request_options) {
		...
		self.cacheStore.get(request_basename, file_extension, ObjectUtils.cacheObj(null, header), request_options, function(err, cache_obj) {
			return self.sendResponse(response, status_code, cache_obj.options.header, cache_obj.body);
		});
	},
	...
	validatePublicCache: function(request, response, stat, callback) {
		...
		if (ConnectUtils.conditionalGET(request)) {
			if (stat) {
				headers = { "etag": ConnectUtils.etag(stat), "last-modified": stat.mtime };
			} else {
	...
	handleBundleRequest: function(request, response, request_basename, file_extension, request_options) {
		...
		console.log(request);
		AssetBundler.getBundle(request_basename, file_extension, request_options, function(err, bundled_obj) {
			if (err) {
	...
	handle: function(request, response, next) {
		...
		if (AssetBundler.isBundlePath(request_basename, file_extension)) {
