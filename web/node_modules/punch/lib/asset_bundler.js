...
var ObjectUtils = require("./utils/object_utils.js");
var PathUtils = require("./utils/path_utils.js");
...
module.exports = {
	...
	setup: function(config) {
		...
		self.cacheStore = ModuleUtils.requireAndSetup(config.plugins.cache_store, config);

		_.each(config.plugins.compilers, function(value, key) {
			self.compilers[key] = ModuleUtils.requireAndSetup(value, config);
		});

		_.each(config.plugins.minifiers, function(value, key) {
			self.minifiers[key] = ModuleUtils.requireAndSetup(value, config);
		});
	...
	prepareBundle: function(bundle, output_extension, callback) {
		...
		var fetch_compile_and_minify = function() {
			if (cloned_bundle_templates.length) {
				...
				var current_template_extension = PathUtils.getExtension(current_template_path);
	...
	getContainedFilesInBundle: function(bundle_name, file_extension, callback) {
		...
		var resolve_defined_paths = function(err, given_paths) {
			...
			if (defined_paths.length) {
				...
				var current_basename = PathUtils.getBasename(current_path, current_extension);
		...
		self.getContainedFilesInBundle(bundle_name, file_extension, function(err, bundled_files) {
			...
			var prepare_bundle_and_update_cache = function() {
				return self.prepareBundle(bundled_files, file_extension, function(minified_output) {
					return self.cacheStore.update(bundle_name, file_extension, ObjectUtils.cacheObj(minified_output, bundle_header), request_options, function(err, cache_obj) {
			...
			self.cacheStore.stat(bundle_name, file_extension, request_options, function(err, stat) {
				...
				var check_for_modified = function() {
					if (cloned_bundled_files.length) {
						...
						return self.cacheStore.get(bundle_name, file_extension, ObjectUtils.cacheObj(null, bundle_header), request_options, function(err, cache_obj) {
							bundle_options.header = cache_obj.options.header;
	...
	statBundle: function(request_basename, file_extension, callback) {
		...
		self.getContainedFilesInBundle(bundle_name, file_extension, function(err, bundled_files) {
			console.log("Name: " + bundle_name);
			console.log(bundled_files);
	...
	fingerprintBundle: function(bundle_path, callback) {
		...
		var bundle_extension = PathUtils.getExtension(bundle_path);
		var bundle_basename = PathUtils.getBasename(bundle_path, bundle_extension);
